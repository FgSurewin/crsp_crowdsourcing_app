// import MarkerIcon from "../../../images/marker.ico";
// import doorIcon from "../../../images/doorIcon.png";
// import doorknobIcon from "../../../images/doorknobIcon.png";
// import stairsIcon from "../../../images/stairsIcon.png";
// import rampIcon from "../../../images/rampIcon.png";
// import { getFov } from "../../../api/staticStreetView";
...
export function CreatePanoMarker(googleMaps) {
  var PanoMarker = function (opts) {
    ...
    if (!opts.container) {
      ...
      console.error("A panorama container needs to be defined.");
    }
    ...
    if (
      pixelCanvas &&
      (pixelCanvas.getContext("experimental-webgl") ||
        pixelCanvas.getContext("webgl"))
    ) {
      this.povToPixel_ = PanoMarker.povToPixel3d;
    }

    /** @private @type {googleMaps.Point} */
    ...
    /** @private @type {?googleMaps.StreetViewPanorama} */
    this.pano_ = null;

    /** @private @type {number} */
    this.pollId_ = -1;

    /** @private @type {googleMaps.StreetViewPov} */
  ...
  PanoMarker.povToPixel3d = function (targetPov, currentPov, zoom, viewport) {
    ...
    // var fov = getFov(zoom) * DEG_TO_RAD;
    console.log("Other FOV -> ", fov);
  ...
  PanoMarker.prototype.onAdd = function () {
    ...
    // var marker = React.createElement("div");
    ...
    // imgElem.src = getIcon(this.labelType_);
    // imgElem.style.width = "20px";

    // marker.appendChild(imgElem);
  ...
  PanoMarker.prototype.draw = function () {
    ...
    console.log("CHECK ZOOM -> ", this.pano_.getPov());
  ...
  //// Getter to be roughly equivalent to the regular googleMaps.Marker ////

  /** @return {googleMaps.Point} The marker's anchor. */
  PanoMarker.prototype.getAnchor = function () {
    return this.anchor_;
  };

  /** @return {string} The className or null if not set upon marker creation. */
  PanoMarker.prototype.getClassName = function () {
    return this.className_;
  };

  /** @return {boolean} Whether the marker is clickable. */
  PanoMarker.prototype.getClickable = function () {
    return this.clickable_;
  };

  /** @return {string} The current icon, if any. */
  PanoMarker.prototype.getIcon = function () {
    return this.icon_;
  };
  ...
  /** @return {googleMaps.StreetViewPanorama} The current panorama. */
  PanoMarker.prototype.getPano = function () {
    return this.pano_;
  };

  /** @return {googleMaps.StreetViewPov} The marker's current position. */
  PanoMarker.prototype.getPosition = function () {
    return this.position_;
  };

  /** @return {googleMaps.Size} The marker's size. */
  PanoMarker.prototype.getSize = function () {
    return this.size_;
  };
  ...
  PanoMarker.prototype.getZIndex = function () {
    return this.zIndex_;
  };

  //// Setter for the properties mentioned above ////

  /** @param {googleMaps.Point} anchor The marker's new anchor. */
   ...
   * StreetViewPanorama.
   *
   * @param {googleMaps.StreetViewPanorama} pano The panorama in which to show
   *    the marker.
  ...
  PanoMarker.prototype.setPano = function (pano, container) {
    ...
    if (!!pano && !(pano instanceof googleMaps.StreetViewPanorama)) {
      console.error("PanoMarker only works inside a StreetViewPanorama.");
  ...
  /** @param {googleMaps.StreetViewPov} position The desired position. */
  PanoMarker.prototype.setPosition = function (position) {
    this.position_ = position;
    this.draw();
  };

  /** @param {googleMaps.Size} size The new size. */
